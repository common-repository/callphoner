...
/**
	...
	Description: Increase calls of the clients who visit your website from the mobile phone
	Author: WPShop.ru
	Version: 1.1.1
...
define('CALLPHONER_VERSION', '1.1.1');
...
class CallPhoner {
	...
	public function __construct() {
		...
		if (isset($_GET['callphoner_debug']) && $_GET['callphoner_debug'] == 1) {
	...
	public function admin_enqueue_scripts($page) {
		if (!$this->wp_version_compare()) {
			wp_deregister_style('dashicons');
			wp_register_style('dashicons', CALLPHONER_PLUGIN_URL.'assets/css/dashicons.min.css');
		}
	...
	public function wp_version_compare($version = 4.1) {
		global $wp_version;
		
		if ($wp_version > $version) return true;
		else return false;
	}
	...
	public function settings_html() {
		...
		$attr = isset($this->settings['attr']) ? $this->settings['attr'] : '';
		...
		<div class="wrap">
			...
			<form method="POST" action="" class="callphoner_form">
				...
				<table class="form-table">
					...
					<tr>
						<th scope="row"><label for="callphoner_tag_attr"><?php _e('Additional button settings', 'callphoner'); ?></label></th>
						<td>
							<input name="callphoner_tag_attr" type="text" id="callphoner_tag_attr" class="regular-text" value="<?=htmlspecialchars($attr);?>">
							<p class="description" id="callphoner-tag-attr-description"><?php _e('You can add your button settings, e.g.: Event Tracking Google Analytics', 'callphoner'); ?></p>
						</td>
					</tr>
					<tr>
	...
	private function saveSettings() {
		if (isset($_POST['callphoner_action']) && $_POST['callphoner_action'] == 'saveSettings' /*&& wp_verify_nonce($_POST['callphoner_nonce'], 'callpnoner_save_settings')*/) {
			...
			$attr = trim($_POST['callphoner_tag_attr']) != '' ? $_POST['callphoner_tag_attr'] : '';
			$schedule = array();
		...
		if (isset($this->settings['status']) && $this->settings['status'] == 'enabled' && $this->settings['tel'] != '') {
			...
			if (isset($this->settings['schedule'][$day_of_week])) {
				...
				if ($show !== false) {
					...
					<a href="tel:<?= $this->settings['tel']; ?>" class="call dashicons dashicons-phone"<?=($this->settings['attr'] != '') ? ' ' . $this->settings['attr'] : ''; ?>></a>
					<?php
				}
			}
		}
